  ...
  */
  ...
  class d_latest_addons {
    ...
    function getOutput() {
      ...
      $addonsCache = new Cache('kuuzu_website-addons-latest');

      if ($addonsCache->exists(360)) {
        $entries = $addonsCache->get();
      } else {
        $response = HTTP::getResponse(['url' => 'https://www.kuuzu.org/index.php?RPC&GetLatestAddons']);
      ...
      $output .= '    <tr>
                        <td class="text-right" colspan="2"><a href="http://addons.kuuzu.org" target="_blank" title="' . HTML::outputProtected(KUUZU::getDef('module_admin_dashboard_latest_addons_icon_site')) . '"><span class="fa fa-fw fa-home"></span></a></td>
                      </tr>
                    </tbody>
                  </table>';
