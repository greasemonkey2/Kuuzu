      ...
      if ( isset($mInfo) ) {
        ...
        if ($action == 'list_new') {
          ...
          if (isset($mInfo->signature) && (list($scode, $smodule, $sversion, $soscversion) = explode('|', $mInfo->signature))) {
            $contents[] = array('text' => '<br />' . HTML::image(KUUZU::linkImage('icon_info.gif'), KUUZU::getDef('image_icon_info')) . '&nbsp;<strong>' . KUUZU::getDef('text_info_version') . '</strong> ' . $sversion . ' (<a href="http://sig.kuuzu.org/' . $mInfo->signature . '" target="_blank">' . KUUZU::getDef('text_info_online_status') . '</a>)');
          }
