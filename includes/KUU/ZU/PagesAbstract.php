...
abstract class PagesAbstract implements \KUU\ZU\PagesInterface
{
    public $data = [];
    ...
    final public function __construct(\KUU\ZU\SitesInterface $site)
    {
    ...
    public function actionExists($action)
    {
        ...
        if (class_exists($class)) {
            if (is_subclass_of($class, 'KUU\ZU\PagesActionsInterface')) {
                return true;
            } else {
                trigger_error('KUU\ZU\PagesAbstract::actionExists() - ' . implode('\\', $action) . ': Action does not implement KUU\ZU\PagesActionInterface and cannot be loaded.');
            }
        }
